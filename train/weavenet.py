...
class Graph:
    ...
    def __init__(self, points_df, list_atoms, feature_col, mol_id):
        ...
        self._array = np.concatenate(
            [one_hot, bonds, self.features], axis=1).astype(np.float32)
...
class WeaveNet(chainer.Chain):
    ...
    def predict(self, input_array, edge_features, pairs_features, **kwargs):
        ...
        edge_features = edge_features.reshape(
            h.shape[0], h.shape[1], h.shape[1], -1)
        concat = F.concat([
            h[pairs_features[:, 0].astype(
                np.int32), pairs_features[:, 1].astype(np.int32), :],
            h[pairs_features[:, 0].astype(
                np.int32), pairs_features[:, 2].astype(np.int32), :],
...
def coupling_converter(batch, device):
    ...
    for i, d in enumerate(batch):
        ...
        list_pairs.append(np.concatenate(
            [sample_index, atom_index, couple_features], axis=1))
    ...
    pairs_features = to_device(
        device, np.concatenate(list_pairs).astype(np.float32))

    # array = {'input_array': input_array, 'dists': dists, 'pairs_features': pairs_features}
    array = {'input_array': input_array,
             'edge_features': edge_features, 'pairs_features': pairs_features}
...
def main():
    ...
    # %%
    # submit = pd.read_csv(f'weavenet_{CTYPE}.csv')
...
if __name__ == "__main__":
    ...
    # %% Load edge features
    edge_mat = joblib.load(
        DATA_PATH / 'edge-feature-matrices' / 'edge_mat_small.pkl')
